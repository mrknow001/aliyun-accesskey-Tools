...
# Form implementation generated from reading ui file 'OSSTools.ui'
#
...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.gridLayout.addWidget(self.pushButton_6, 10, 5, 1, 1)
    ...
    def retranslateUi(self, MainWindow):
        ...
        item.setText(_translate("MainWindow", "网卡类型："))
        ...
        self.pushButton_5.setText(_translate("MainWindow", "自动填入"))
        self.pushButton_6.setText(_translate("MainWindow", "命令回显"))
...
def Echo_Command(ui, echo_id):
    ...
    class MyThread(QThread):
        ...
        def run(self):
            Check_ComText(echo_id)

    thread = MyThread()
    uc.pushButton.clicked.connect(partial(thread.start))


def Check_ComText(echo_id):
    uc.pushButton.setEnabled(False)
    uc.pushButton.setText("请等待")
    com = uc.lineEdit.text()
    try:
    ...
    if com == "":
        ...
        show_message(ui, "请输入实例ID,或者选中实例ID")
        uc.pushButton.setEnabled(True)
        uc.pushButton.setText("执行")
    else:
        print(InstanceId)
        InstanceIP = ui.tableWidget.item(InstanceId_row, 6).text()
        print(InstanceIP.strip(";"))
        Ui_C.setWindowTitle(InstanceId + "(" + InstanceIP.strip(";") + ")")

        function.commad_check_input(ui,uc,echo_id)
...
if __name__ == "__main__":
    ...
    ui = Ui_MainWindow()
    uc = command.Ui_Commond()
    ui.setupUi(widget)
    uc.setupUi(Ui_C)
    widget.setWindowTitle("Aliyun- AK Tools By T00ls.Net")
